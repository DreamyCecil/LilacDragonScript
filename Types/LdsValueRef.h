...
#define CONSTRUCTOR_FULL(_Type) SLdsValueRef(const _Type &_Set, SLdsVar *_Var, SLdsValue *_Access, const string &_Name, const string &_Ref, bool _Const) : \
                                             val(_Set), ARGS_TEMP(_Var, _Access, _Name, _Ref, _Const) {}

// Value reference
struct LDS_API SLdsValueRef {
  SLdsValue val; // value itself
  
  SLdsVar *pvar; // variable reference
  SLdsValue *pvalAccess; // array/structure value reference
